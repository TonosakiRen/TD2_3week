...
void BossExplode::Draw(ViewProjection* viewProjection, DirectionalLight* directionalLight, Vector4 color,uint32_t textureHandle)
{
	...
	if (!instancingBufferDatas.empty()) {
		particleBox_->Draw(instancingBufferDatas, *viewProjection, *directionalLight, color, textureHandle);
	}
}
